...
JumpFloodingShader::JumpFloodingShader(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"jumpFlooding_cs.cso", NULL);
}
...
void JumpFloodingShader::initShader(const wchar_t* cfile, const wchar_t* blank)
{
...
void JumpFloodingShader::createOutputUAV()
{
...
void JumpFloodingShader::setShaderParameters(ID3D11DeviceContext* dc, ID3D11ShaderResourceView* penetrationMap, int step)
{
...
void JumpFloodingShader::unbind(ID3D11DeviceContext* dc)
{
