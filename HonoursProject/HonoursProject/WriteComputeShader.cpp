...
WriteComputeShader::WriteComputeShader(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"write_compute_cs.cso", NULL);
}
...
void WriteComputeShader::initShader(const wchar_t* cfile, const wchar_t* blank)
{
	loadComputeShader(cfile);
	createOutputUAV();
}

void WriteComputeShader::createOutputUAV()
{
...
void WriteComputeShader::setShaderParameters(ID3D11DeviceContext* dc,ID3D11ShaderResourceView* displacementMap)
{
	dc->CSSetShaderResources(0, 1, &displacementMap);
	dc->CSSetUnorderedAccessViews(0, 1, &m_uavAccess, 0);
}

void WriteComputeShader::unbind(ID3D11DeviceContext* dc)
{
