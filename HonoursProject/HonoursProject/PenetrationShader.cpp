...
PenetrationShader::PenetrationShader(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"penetration_cs.cso", NULL);
}
...
void PenetrationShader::initShader(const wchar_t* cfile, const wchar_t* blank)
{
	loadComputeShader(cfile);
	createOutputUAV();
}

void PenetrationShader::createOutputUAV()
{
...
void PenetrationShader::setShaderParameters(ID3D11DeviceContext* dc, ID3D11ShaderResourceView* heightMap, ID3D11ShaderResourceView* deformationMap)
{
...
void PenetrationShader::unbind(ID3D11DeviceContext* dc)
{
