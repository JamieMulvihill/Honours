...
VolumePreservationPongShader::VolumePreservationPongShader(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"volumePreservationPong_cs.cso", NULL);
}
...
void VolumePreservationPongShader::initShader(const wchar_t* cfile, const wchar_t* blank)
{
...
void VolumePreservationPongShader::createOutputUAV()
{
...
void VolumePreservationPongShader::setShaderParameters(ID3D11DeviceContext* dc, ID3D11ShaderResourceView* heightMap, ID3D11ShaderResourceView* displacementMap, float treshold)
{
...
void VolumePreservationPongShader::unbind(ID3D11DeviceContext* dc)
{
